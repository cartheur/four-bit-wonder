...
| 1 | Advance pushbutton | E-Switch [`PS1024`, circuit A](../datasheets/PS1024.pdf), SPST `OFF-(ON)` | Optional manual `NEXT ADDRESS` control for test and demonstration. |
| 1 | Debounce resistor | `10k` pull-down; inventory has 100 | Holds the button input low until the normally-open contact closes. |
| 1 | Debounce capacitor | `100nF` from the button input to ground | Debounces the close-on-press transition before the `74LS14`. |
...
Wire the normally-open `NEXT ADDRESS` contact from `+5V` to the button input.
At rest the `10k` pull-down holds the input low; pressing drives it high. The
first `74LS14` inverter therefore produces an active-low
